      ...
      const fetchRelation = async () => {
        try {
          const response = await axios.get<Relation>(
            `http://localhost:${process.env.REACT_APP_CLIENT_PORT || 7033}/api/Familytrees/${familyTreeId}/persons/${selectedEdge.personId_1}/relations/${selectedEdge.id}`,
            config
          );
      ...
      if (selectedEdge) {
        ...
        if (selectedPersonInTree) {
          ...
          const response = await axios.post(
            `http://localhost:${process.env.REACT_APP_CLIENT_PORT || 7033}/api/Familytrees/${familyTreeId}/persons/${selectedPersonInTree[1]}/relations`,
            {
              ...formData,
              startDate: formData.startDate.split("T")[0],
              endDate: formData.endDate
