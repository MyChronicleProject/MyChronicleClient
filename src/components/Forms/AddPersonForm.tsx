...
export default function AddPersonForm({
      ...
      const fetchPerson = async () => {
        try {
          ...
          const response = await axios.get<Person>(
            `http://localhost:${process.env.REACT_APP_CLIENT_PORT || 7033}/api/Familytrees/${familyTreeId}/persons/${selectedNode}`,
            config
          );
      ...
      if (selectedNode) {
        ...
        if (profilePictureToSend) {
          const responseFoto = await axios.post(
            `http://localhost:${process.env.REACT_APP_CLIENT_PORT || 7033}/api/Familytrees/${familyTreeId}/persons/${selectedNode}/files`,
            {
          ...
          if (responseFoto.status === 200 || responseFoto.status === 201) {
            ...
            try {
              if (profileFile) {
                ...
                const response = await axios.put(
                  `http://localhost:${process.env.REACT_APP_CLIENT_PORT || 7033}/api/Familytrees/${familyTreeId}/persons/${selectedNode}/files/${profileFile.id}`,
                  {
                    ...profileFile,
        ...
        if (response.status === 200 && response.data) {
          ...
          if (profilePictureToSend) {
            const responseFoto = await axios.post(
              `http://localhost:${process.env.REACT_APP_CLIENT_PORT || 7033}/api/Familytrees/${familyTreeId}/persons/${updatedFormData.id}/files`,
              {
                name: profilePictureToSend.name,
                fileType: getFileTypeNumber(profilePictureToSend.fileType),
                content: profilePictureToSend.content,
  ...
  const handleDeleteFile = (fileToDelete: File) => {
    ...
    axios
      .delete(
        `http://localhost:${process.env.REACT_APP_CLIENT_PORT || 7033}/api/Familytrees/${familyTreeId}/persons/${fileToDelete.personId}/files/${fileToDelete.id}`,
        config
      )
      .then(() => {
        setFiles((prevFiles) =>
